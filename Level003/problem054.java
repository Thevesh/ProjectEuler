...
public class problem054 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 1000; i++){
			if (OneWins(Parse(Games[i]))) count++;
		}
	...
	// First we are going to sort out the hands. This took about 0.02 seconds to run with no checking of anything.
	
 	public static boolean OneWins (String[][] n){
 		
 		// We'll need to (a) Sort the matrix into numbers and suits (b) Define values of letters (c) Convert to Int arrays.
 		
 		String[] handOne = n[0];
 		String[] handTwo = n[1];
 		String[] numbersOne = new String[5];
 		String[] numbersTwo = new String[5];
 		String[] suitsOne = new String[5];
 		String[] suitsTwo = new String[5];
 		
 		for (int i = 0; i < 5; i++){
 		 numbersOne[i] = String.valueOf(handOne[i].charAt(0));
 		 numbersTwo[i] = String.valueOf(handTwo[i].charAt(0));
 		 suitsOne[i] = String.valueOf(handOne[i].charAt(1));
 		 suitsTwo[i] = String.valueOf(handTwo[i].charAt(1));
 		}
 		
 		for (int i = 0; i < 5; i++){
	 		 if(numbersOne[i].equals("T")) numbersOne[i] = "10";
 	 		 if(numbersOne[i].equals("J")) numbersOne[i] = "11";
 	 		 if(numbersOne[i].equals("Q")) numbersOne[i] = "12";
 	 		 if(numbersOne[i].equals("K")) numbersOne[i] = "13";
 	 		 if(numbersOne[i].equals("A")) numbersOne[i] = "14";
	 		 if(numbersTwo[i].equals("T")) numbersTwo[i] = "10";
 	 		 if(numbersTwo[i].equals("J")) numbersTwo[i] = "11";
 	 		 if(numbersTwo[i].equals("Q")) numbersTwo[i] = "12";
 	 		 if(numbersTwo[i].equals("K")) numbersTwo[i] = "13";
 	 		 if(numbersTwo[i].equals("A")) numbersTwo[i] = "14";
 	 		}
 		
 		for (int i = 0; i < 5; i++){
	 		 if(suitsOne[i].equals("D")) suitsOne[i] = "1";
	 		 if(suitsOne[i].equals("C")) suitsOne[i] = "2";
	 		 if(suitsOne[i].equals("H")) suitsOne[i] = "3";
	 		 if(suitsOne[i].equals("S")) suitsOne[i] = "4";
	 		 if(suitsTwo[i].equals("D")) suitsTwo[i] = "1";
	 		 if(suitsTwo[i].equals("C")) suitsTwo[i] = "2";
	 		 if(suitsTwo[i].equals("H")) suitsTwo[i] = "3";
	 		 if(suitsTwo[i].equals("S")) suitsTwo[i] = "4";
	 		}
 		...
 		for (int i = 0; i < 5; i++){
 			n1[i] = Integer.valueOf(numbersOne[i]);
 			n2[i] = Integer.valueOf(numbersTwo[i]);
 			s1[i] = Integer.valueOf(suitsOne[i]);
 			s2[i] = Integer.valueOf(suitsTwo[i]);
 		}
 		...
 		// Everything up to this point was just putting the hands into usable arrays. Now, we begin defining the game.
 		
 		if (RoyalFlush(n1,s1) > RoyalFlush(n2,s2)) return true; 	
 		else if (RoyalFlush(n1,s1) < RoyalFlush(n2,s2)) return false; 
 		else if (StraightFlush(n1,s1) > StraightFlush(n2,s2)) return true; 	
 		else if (StraightFlush(n1,s1) < StraightFlush(n2,s2)) return false; 			
 		else if (FourOfAKind(n1) > FourOfAKind(n2)) return true; 	 			
 		else if (FourOfAKind(n1) < FourOfAKind(n2)) return false; 		
 		else if (FullHouse(n1) > FullHouse(n2)) return true; 	
 		else if (FullHouse(n1) < FullHouse(n2)) return false; 		
 		else if (Flush(s1) > Flush(s2)) return true; 
 		else if (Flush(s1) < Flush(s2)) return false; 
 		else if (Straight(n1) > Straight(n2)) return true; 
 		else if (Straight(n1) < Straight(n2)) return false; 		
 		else if (ThreeOfAKind(n1) > ThreeOfAKind(n2)) return true;	
 		else if (ThreeOfAKind(n1) < ThreeOfAKind(n2)) return false; 	 		
 		else if (TP1[0] > TP2[0]) return true;
 		else if (TP1[0] < TP2[0]) return false;	 		
 		else if (TP1[1] > TP2[1]) return true;
 		else if (TP1[1] < TP2[1]) return false;
 		else if (OnePair(n1) > OnePair(n2)) return true; 
 		else if (OnePair(n1) < OnePair(n2)) return false;
 		
 		for (int i = 0; i < 5; i ++){
 			if (HC1[i] > HC2[i]) return true;	
 			if (HC1[i] < HC2[i]) return false; 
 		}	
 		return false;
 	}
 	...
 	// Now, we begin strength testing, going down the order and defining a separate boolean for each one.
 	
 	
 	// Test for Royal Flush. Dependent upon test for Flush and Straight. 	
 	public static int RoyalFlush(int[] n, int[] s){
 		int straight = Straight(n);
 		if (straight != 14) return 0;
 		else return Flush(s);
 	}
 	
 	
 	
 	// Test for Straight Flush. Dependent upon test for Flush and Straight.
 	public static int StraightFlush(int[] n, int[] s){
 		int straight = Straight(n);
 		int flush = Flush(s);
 		
 		if (straight == 0 || flush == 0) return 0;
 	...
 	public static int FourOfAKind(int[] n){
 		Set<Integer> unique = new HashSet<Integer>();
 	    for (int val : n) {
 	        unique.add(Integer.valueOf(val));
 	    }
 	...
 	public static int Straight(int[] n){
 	   ...
 	   if (unique.size() != 5) return 0;
 		   ...
 		   Arrays.sort(n);
 		   if (n[4] - n[0] == 4) return n[4];
 		   if (unique.contains(14) && n[3] + n[0] == 7 && n[3] - n[0] == 3) return 5;
 	...
 	public static int ThreeOfAKind (int[] n){
 		int count = 0;
 		Set<Integer> unique = new HashSet<Integer>();
 		
 	    for (int val : n) {
 	        if (unique.add(val) == false) count ++;
 	        unique.add(Integer.valueOf(val));
 	    }
 	    
 	    if (unique.size() != 2 && unique.size() != 3) return 0;
 	    if (count != 3) return 0;
 	    Arrays.sort(n);
 	    return n[2];
 	}
 	...
 	// Test for Two Pairs
 	public static int[] TwoPairs (int[] n){
 	...
 	// Test for One Pair
  	public static int OnePair (int[] n){
  	    ...
  	    for (int val : n) {
  	        if (unique.add(val) == false) count += val;
  	        unique.add(Integer.valueOf(val));
  	    }
  	...
  	// Test for High Card.
 	public static int[] HighCard(int[] n){
	...
	public static String[][] Parse(String m){
		String[][] Values = new String[2][5];
		
		for (int i = 0; i < 13; i+=3){
			int place = i/3;
			String temp = String.valueOf(m.charAt(i)) + String.valueOf(m.charAt(i+1));
			Values[0][place] = temp;
		}
	...
	public static int[][] Transpose(int[] n, int[] s){
	...
	private static String[] TestGames = {
			"5H 5C 6S 7S KD 2C 3S 8S 8D TD",
			"5D 8C 9S JS AC 2C 5C 7D 8S QH",
			"2D 9C AS AH AC 3D 6D 7D TD QD",
			"4D 6S 9H QH QC 3D 6D 7H QD QS",
			"2H 2D 4C 4D 4S 3C 3D 3S 9S 9D",
			"AS 2S 3S 4S 5S 6H 6C 6D 2D 3D",
		};
